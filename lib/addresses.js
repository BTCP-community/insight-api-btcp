...
const btcpP2PKHVersionBytes = Buffer.from([0x13, 0x25]);
const btcpP2SHVersionBytes = Buffer.from([0x13, 0xaf]);

function btcpify(zclOrBtcAddress) {
  try {
    let raw = bitcore.encoding.Base58Check.decode(zclOrBtcAddress);

    let first = raw.slice(0,1).toString('hex');
    if (first === '00' || first === '05') { // BTC
      let v = (first === '00') ? btcpP2PKHVersionBytes : btcpP2SHVersionBytes;
      let newBuffer = Buffer.concat([v, raw.slice(1)]);
      return bitcore.encoding.Base58Check.encode(newBuffer).toString();
    } else {
      let firstTwo = raw.slice(0,2).toString('hex')
      if (firstTwo === '1325' || firstTwo === '13af') { // BTCP
	return zclOrBtcAddress;
      } else if (firstTwo === '1cb8' || firstTwo === '1cbd') { // ZCL
        let v = (firstTwo === '1cb8') ? btcpP2PKHVersionBytes : btcpP2SHVersionBytes;
	let newBuffer = Buffer.concat([v, raw.slice(2)]);
	return bitcore.encoding.Base58Check.encode(newBuffer).toString();
      } else {
...
AddressController.prototype.check = function(req, res, next, addresses) {
  ...
  for(var i = 0; i < addresses.length; i++) {
    try {
      var btcpAddress = btcpify(addresses[i])
      //console.log(btcpAddress);
      var a = new bitcore.Address(btcpAddress);
    } catch(e) {
      return self.common.handleErrors({
